...
* The value for `SHA2_384_PAYONE_KEY` needs to be the SHA2-384 encryption result of your PAYONE key (`PAYONE_KEY`). 
Use `echo -n $PAYONE_KEY | shasum -a 384` to hash string value to SHA2-384.
...
and _shasum_ command is available (which is default case on Mac OS X and most of Linux distributions), 
you may copy-paste and directly execute next line (change only `<VISA_CREDIT_CARD_3DS_NUMBER>`):

```
curl --data "request=3dscheck&mid=$PAYONE_MERCHANT_ID&aid=$PAYONE_SUBACC_ID&portalid=$PAYONE_PORTAL_ID&key=$(echo -n $PAYONE_KEY | shashum -a 384)&mode=test&api_version=3.9&amount=2&currency=EUR&clearingtype=cc&exiturl=http://www.example.com&storecarddata=yes&cardexpiredate=2512&cardcvc2=123&cardtype=V&cardpan=<VISA_CREDIT_CARD_NUMBER>" https://api.pay1.de/post-gateway/
```
